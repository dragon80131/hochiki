...
$wSignCD = SPFWParameter::getValues('SignCD');

if($work == '3'){
	$mySign = new Sign($myDB);
	if (!$mySign->executeSelect(" SignCD = '".$wSignCD."'", "")) {
		trigger_error("Getting Sign Failed.", E_USER_ERROR);
	}
	$mySign->MukouFlg = TRUE;
	$mySign->Updater = $UserCD;
    ...
    if($editBuildingCD){
        if (!$myReservation->executeSelect(" TimeFrom >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH) AND  BukkenCD = '".$editBukkenCD."' AND BuildingCD = '".$editBuildingCD."' AND ID = '".$mySign->ID."' AND MukouFlg = FALSE", "")) {
            trigger_error("Getting Bukken Failed.", E_USER_ERROR);
        }
    }else{
        if (!$myReservation->executeSelect(" TimeFrom >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH) AND  BukkenCD = '".$editBukkenCD."' AND BuildingCD IS NULL AND ID = '".$mySign->ID."' AND MukouFlg = FALSE", "")) {
            trigger_error("Getting Bukken Failed.", E_USER_ERROR);
        }
    }
    $myReservation->KanryoFlg = 0;
...
if($BuildingLoop > 0){
	...
	$IfBuildingNotExist = false;
}else{
	$IfBuildingExist = false;
	$IfBuildingNotExist = true;
}


$WakuPattern = $myBukken->WakuPattern;
$Kaidaka = $myBukken->Kaidaka;
if($editBuildingCD){
	$WakuPattern = $myBukken->WakuPattern;
	$Kaidaka = $myBuilding->Kaidaka;
}

$TargetClientCD = $myBuilding->ClientCD; #111


unset($myBukken);

$wKaidaka = SPFWParameter::getValues('wKaidaka'); // 階数
$KaidakaLoop = intval($Kaidaka);
for ($i = 1; $i <= $KaidakaLoop; $i++) {
	$aKaidaka[$i-1] = $i;
	if($i == $wKaidaka)
		$aKaidakaSelected[$i-1] = "selected";
	else
		$aKaidakaSelected[$i-1] = "";
}
...
if($wKaidaka){ // 階数による絞り込み
	$sql .= " AND r.ID LIKE CONCAT('".$wKaidaka."', '__')";
}
...
for ($i = 0; $i < $ResidentsFormLoop; $i++) {
	...
	$SignCD = $myListObject->GetValue($i,5);

	$RefugeFlg = $myListObject->GetValue($i,6);
	if($RefugeFlg == '1')
		$RefugeFlgChecked[$i] = ' checked';
	else
		$RefugeFlgChecked[$i] = '';

	$Created[$i] = str_replace("-","/",substr($myListObject->GetValue($i, 4),0,16));
	if($KanryoFlg[$i] == 1){#完了書がある
		$Kanryosyo[$i] ="<a href=".$FilePath[$i]."><img src=".$FilePath[$i]." height='50px'></a>";
		$KanryosyoColor[$i] =" bgcolor ='#f0f8ff' ";
		$KanryosyoDel[$i] = "<button type='button' class='btn btn-danger btn-sm' onclick=\"del_sign('s_kanryo.php?rKey=".$rKey."&SignCD=".$SignCD."&editBukkenCD=".$editBukkenCD."&editBuildingCD=".$editBuildingCD."&work=3');\">削除</button>";
	}else{
		$Kanryosyo[$i] ="<a href='#' onclick=\"javascript:moveWithKey('s_sign.php?rKey=".$rKey."&ID=".$ID[$i]."&m=".$m."&editBuildingCD=".$editBuildingCD."',".$editBukkenCD." )\">未</a>";
 		// $Kanryosyo[$i] ="未";
		$KanryosyoDel[$i] = '';
	}
}

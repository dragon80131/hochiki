...
########################################################
# 点検報告formのリセット(2か月後に点検が控えている物件が対象)
########################################################
...
for ($j = 0; $j < $TargetBukkenLoop; $j++) {
	...
	$Date1[$j] = $myListObject->GetValue($j, 1);
	if ($TargetBukkenCD[$j]) {
		...
		// $statement = 'UPDATE tResidentsFormF SET Moved_at = "' . $Date1[$j] . '" WHERE BukkenCD = "' . $TargetBukkenCD[$j] . '"';
		$statement = 'UPDATE tResidentsFormF SET Moved_at = "' . $Date1[$j] . '" WHERE BukkenCD = "' . $TargetBukkenCD[$j] . '" AND Moved_at IS NULL';
		$stmt = $dbh->prepare($statement);
		$stmt->execute();
		$reg_array = $stmt->fetchAll();

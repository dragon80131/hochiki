...
*/
	...
	$spreadsheet = $reader->load('./template/s_koji_annai_Excel_v2.xlsx');
...
######### 1_概要 #########
	...
	$sheet->setCellValue('A'.($gyo_sheet1 + 2), $BukkenName.'にお住まいの皆様へ'); // 2行目

	#管理会社・アイホン問合せ先
	#◆右上に表示する会社名
	$sheet->setCellValue('AN'.($gyo_sheet1 + 3), $DispCompanyTop1); // 3行目
	$sheet->setCellValue('AN'.($gyo_sheet1 + 4), $DispCompanyTop2); // 4行目
	$sheet->setCellValue('AN'.($gyo_sheet1 + 5), $DispCompanyTop3); // 5行目
	...
	if($nonTouchFLG){
		...
		$sheet->setCellValue('Z'.($gyo_sheet1 + 23), "各住戸".$wTagSuu."本"); // 23行目
		...
		if ($wAutoLock == "1"){ #AutoLockありなら
			if ($wKirikaehoho == '0'){	#切替方法→停止
				...
				if ($wKansenKoji == "2") { #幹線ルート部屋渡りの場合
					...
					$drawings = $sheet->getDrawingCollection();
					foreach ($drawings as $key=>$drawing){
						if( ($drawing->getName() === '3_中NEW集合玄関機') OR 
							($drawing->getName() === '3_中NEW右矢印丸') OR
							($drawing->getName() === '3_中NEW親機') OR
							($drawing->getName() === '3_中NEW左矢印丸') OR
...
######### 4_仮暗証番号操作方法 #########
	...
	#ベスト腕章表示処理
	if (!is_array($wSagyoinArr)) { // ベスト・腕章なし

...
*/
	...
	$spreadsheet = $reader->load('./template/s_koji_annai_Excel_v3.xlsx');//template.xlsx 読込

	$sheet = $spreadsheet->getActiveSheet();

	$sheet->setCellValue('B3', $BukkenName . "にお住まいの皆様へ");
  $sheet->setCellValue('W1',$AnnaiDateDisp);//工事資料配布日
  $sheet->setCellValue('W4','管理会社：'.$KanriGaisya);//管理会社
  $sheet->setCellValue('W5','施工会社：'.$SekoShutai);//施工会社の主体？
	...
	#【工事お問い合わせ】
  $sheet->setCellValue('V91', '施工会社：'.$SekoShutai.'  担当：'.$KojiTantoName);
  $sheet->setCellValue('V92', '電話：'.$TantoTEL);

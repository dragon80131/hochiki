...
if ($editBukkenCD > 0) { #物件情報の修正の場合
	...
	function dispPic2($PictureCD)
	{
		...
		// 表示するイメージのGenbaCDをパラメータから取得
		// サムネイル
		$sql = 'SELECT PictureCD,P002,BukkenCD FROM tPictureF WHERE PictureCD = ' . $PictureCD . ' and MukouFlg = 0';
		// データの取得
		$result = $mysqli->query($sql);
		$row = $result->fetch_assoc();

		$enc_img = base64_encode($row["P002"]);

		$imginfo = getimagesize('data:application/octet-stream;base64,' . $enc_img);

		//echo '<img src="data:' . $imginfo['mime'] . ';base64,'.$enc_img.'">';
		return '<img src="data:' . $imginfo['mime'] . ';base64,' . $enc_img . '" style=width:45px >';
	}

	function dispPic($PictureCD)
	{
		include_once "setting.properties";
		include_once _INC_DIR . "carrier.inc";
		include_once _INC_DIR . "global.inc";
		...
		// 表示するイメージのGenbaCDをパラメータから取得
		// サムネイル
		$sql = 'SELECT PictureCD,Pic,BukkenCD FROM tPictureF WHERE PictureCD = ' . $PictureCD . ' and MukouFlg = 0';
		// データの取得
		$result = $mysqli->query($sql);
		$row = $result->fetch_assoc();

		$enc_img = base64_encode($row["Pic"]);

		$imginfo = getimagesize('data:application/octet-stream;base64,' . $enc_img);

		//echo '<img src="data:' . $imginfo['mime'] . ';base64,'.$enc_img.'">';
		return '<img src="data:' . $imginfo['mime'] . ';base64,' . $enc_img . '" style=width:90px >';
	}
	for ($i = 0; $i < $PicLoop; $i++) {
		...
		#		if (mb_strlen( $Memo[$i] ) > 5) {
		#			$Memo[$i] = mb_substr($Memo[$i], 0, 5);
		#			$Memo[$i] = $Memo[$i]."...";
		#		}

		$tmp = "<input type='checkbox' name='heyapic[]' value='" . $PictureCD[$i] . "'><a href='../viewpic.php?BukkenCD=" . $editBukkenCD . "&Device=" . $Device[$i] . "&PictureCD=" . $PictureCD[$i] . "' TARGET='_blank'>";

		$tmp .= dispPic2($PictureCD[$i]) . "</a>";

		$TestPic = dispPic($PictureCD[$i]);

		#tmp .="<input type=checkbox name=chk[] value=$PictureCD[$i]>";
		//		$tmp .= "<p style='margin:2px; padding:0px; font-size:9px;'>" . $Memo[$i] . "</p>";
...
$CNT_FILE = "s_make_kanryo_test.tpl";

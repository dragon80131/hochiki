...
#機器設定指示書 VIXUS1 登録処理
	...
	for($i=1 ; $i < 56 ; $i++){
		...
		if($i<2)#★★
			${"wDenkijoSettei".$i} = SPFWParameter::getValues("wDenkijoSettei".$i);
		if($i<6)#★
...
#echo "6";
	$myKikiSettei = new KikiSettei($myDB);
	#物件CD　機器CD=4:VIXUS1Pr　を指定してレコードを検索
...
#echo "$wKikiSetteiCD＝＝＝".$wKikiSetteiCD;
	if($wKikiSetteiCD > 0){
		...
		$myKikiSettei->KikiCD = 4;#VIXUS1Pr
	...
	for($i=1 ; $i<56 ;$i++){
		...
		if($i<2)#★★
			$myKikiSettei->{"DenkijoSettei".$i} = ${"wDenkijoSettei".$i};
		if($i<6)#★
	...
	$CNT_FILE = "s_kikiseteiVIXUS1Pr_finish.tpl";

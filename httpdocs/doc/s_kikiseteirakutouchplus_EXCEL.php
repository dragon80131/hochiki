...
//住宅情報盤設定
	...
	$spreadsheet = $reader->load('./template/kikisettei_rakutouchplus.xlsx'); //template.xlsx 読込
	...
	$sheet = $spreadsheet->getSheetByName('親機設定表'); //weatherシート取得
		...
		$sheet->setCellValue($JUTAKUSERVICESETTEI[1], str_replace("<br>","\n",$wJutakuServiceSettei1Name));
		$sheet->setCellValue($JUTAKUSERVICESETTEI[2], str_replace("<br>","\n",$wJutakuServiceSettei2Name));
		$sheet->setCellValue($JUTAKUSERVICESETTEI[3], str_replace("<br>","\n",$wJutakuServiceSettei3Name));
		$sheet->setCellValue($JUTAKUSERVICESETTEI[4], str_replace("<br>","\n",$wJutakuServiceSettei4Name));

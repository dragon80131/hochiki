...
#		$myDBeach = new SPFWDatabase( 'koji'.$BukkenCD489, '192.168.98.221', _USER_NAME, _PASSWD , FALSE);
		$myDBeach = new SPFWDatabase( 'skoji', '192.168.98.221', _USER_NAME, _PASSWD , FALSE);
		...
		$sql .= " WHERE r.UserCD = u.UserCD and r.Notes !='' and r.Status = 1 and u.ClientCD = $wClientCD and r.ClientCD = $wClientCD";
...
#		$sql .= " where exists (select ID from tPictureF b where a.ID = b.ID AND b.MukouFlg = 0 AND b.Device = 1 AND b.SekoStatus = 2 ) ";
		...
		$sql = "SELECT u.ID "; 
		$myListObject3->SelectSQL = $sql;
		$sql = " From tUserM u, tPictureF p ";#　施工後の親機を対象とする　施行前の玄関子機だけ写真とるときがある
		$sql .= " where u.ID = p.ID AND p.MukouFlg = 0 AND p.Device = 1 AND p.SekoStatus = 2 and u.ClientCD = $wClientCD and p.ClientCD = $wClientCD";
		...
		if ($KanID) {
			...
			$sql = " FROM tUserM ";
			$sql .= " WHERE  ClientCD = $wClientCD and ID not like 'dummy%' and ID != 1234 and ID != 5678 and ID !='aiphone' and ID !='kanri' and MukouFlg = false";
			...
			if (!($myListObject4->GetList(1)))
				trigger_error("Getting User List Failed.", E_USER_ERROR);
#echo "<br>".__LINE__."行目:".$RecoLoop;
			$RecoLoop = $myListObject4->Rows;
			for ($j = 0; $j < $RecoLoop; $j++) {
				$ID[$j] = $myListObject4->GetValue($j,0);

				if(!in_array( $ID[$j] , $KanID )){	#完了リストに存在しないIDは未完として避ける
					$MiKanID[] = $ID[$j];
				}
			}
			unset($myListObject4);
			...
			if( count($MiKanID) > 0 ){
				...
				print_r($MiKanID);
				$outputMikanArr = array_slice($MiKanID,0,3);
				for($tmp_i = 0; $tmp_i < 3-count($outputMikanArr);$tmp_i++){
					$outputMikanArr[] = "";
				}
				
				$outputMikanNum = count($MiKanID)-3 > 0 ?  count($MiKanID)-3:0 ;
				/*
				for($k = 0 ; $k < count($MiKanID); $k++){
					if ($ZanHeyaDisp) $ZanHeyaDisp = $ZanHeyaDisp.", ".$MiKanID[$k];	#2件目以降
					else $ZanHeyaDisp = $MiKanID[$k];	#1件目
...
#echo "<br>".__LINE__."行目:".$ZanHeyaDisp;
		#showSorryPage(_ILLEGAL_ACCESS);
	...
	$spreadsheet = $reader->load('./template/kojikanryoA.xlsx'); //template.xlsx 読込
	...
	$sheet->setCellValue('G22', $Kosu); // 総住戸数
	
	if( count($MiKanID) > 0 ){ #未完了住戸があれば
		#$sheet->setCellValue('F25', $ZanHeyaDisp); // 未施工住戸
		$sheet->setCellValue('H24', $outputMikanArr[0]); // 未施工住戸1
		$sheet->setCellValue('L24', $outputMikanArr[1]); // 未施工住戸2
		$sheet->setCellValue('P24', $outputMikanArr[2]); // 未施工住戸3
		$sheet->setCellValue('U24', $outputMikanNum); // 未完住戸その他個数
	...
	$wFileName = "工事完了報告書A_".$BukkenName.".xlsx" ;

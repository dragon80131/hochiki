...
if ($SekoShutai == "株式会社イオ・ネットワーク" or $SekoShutai == "ヴェリアス（株）" or $SekoShutai == "株式会社日本ロックサービス" or $SekoShutai == "東京総合エンジニアリング（株）" or $SekoShutai == "（株）セーフティ＆ベル" or $SekoShutai == "幸和電気(株)" or $SekoShutai == "富永電機（株）" or $SekoShutai == "株式会社産業機電" or $SekoShutai == "共同通信電設株式会社" or $SekoShutai == "（株）たけでん" or $SekoShutai = "大東通工（株）") {
	$TelNumber = "0120-346-131";
} else {
	$TelNumber = "0120-489-501";
}
...
$wHansu 			= $myKoji->Hansu;			#班数
$wMinuteTime 		= $myKoji->MinuteTime;		#工事施工時間（リアル）
$wWakuPattern 		= $myKoji->WakuPattern;	#枠パターン
$wKojijun 			= $myKoji->Kojijun;		#工事順
$wFirstDateFeature 	= $myKoji->FirstDateFeature;		#初日工事数考慮
$wWakuSu 			= "|" . str_replace("-", "|", $myKoji->MaxWakuSu) . "|";		#最大工事枠数
...
if ($wWakuPattern == 8) { //5枠
	...
	$wWakuPM2 	= $wWakuSu[3];
	$wWakuPM3 	= $wWakuSu[4];
} elseif ($wWakuPattern > 2 and $wWakuPattern <= 7) { //3枠
	$wWakuAM	= $wWakuSu[0];
	$wWakuPM1 	= $wWakuSu[1];
	$wWakuPM2 	= $wWakuSu[2];
...
$wHoliday1 		= $myKoji->Holiday1;			#休工日[配列]
...
if ($wWakuPattern > 7) {
	//セルの結合
	$cellAM1 = getExcelAddress(($wWakuAM1col + 3), 8); #列　行　
	$sheet->mergeCells('D8:' . $cellAM1);
	$cellAM2c = getExcelAddress(($wWakuAM1col + 4), 8); #列　行　
	$cellAM2r = getExcelAddress(($wWakuAM1col + 3 + $wWakuAM2col), 8); #列　行
	...
	$aAM3 = $WAKUPATTERN[$wWakuPattern]['StartTime'][2];
	$eAM3 = $WAKUPATTERN[$wWakuPattern]['EndTime'][2];
	$DispAM3 = $aAM3 . "～" . $eAM3;
	$sheet->setCellValue($cellAM3c, $DispAM3);

	$aAM4 = $WAKUPATTERN[$wWakuPattern]['StartTime'][3];
	$eAM4 = $WAKUPATTERN[$wWakuPattern]['EndTime'][3];
	$DispAM4 = $aAM4 . "～" . $eAM4;
	$sheet->setCellValue($cellAM4c, $DispAM4);

	$aPM1 = $WAKUPATTERN[$wWakuPattern]['StartTime'][4];
	$ePM1 = $WAKUPATTERN[$wWakuPattern]['EndTime'][4];
	$DispPM1 = $aPM1 . "～" . $ePM1;

	$aPM2 = $WAKUPATTERN[$wWakuPattern]['StartTime'][5];
	$ePM2 = $WAKUPATTERN[$wWakuPattern]['EndTime'][5];
	$DispPM2 = $aPM2 . "～" . $ePM2;

	$aPM3 = $WAKUPATTERN[$wWakuPattern]['StartTime'][6];
	$ePM3 = $WAKUPATTERN[$wWakuPattern]['EndTime'][6];
	$DispPM3 = $aPM3 . "～" . $ePM3;

	$aPM4 = $WAKUPATTERN[$wWakuPattern]['StartTime'][7];
	$ePM4 = $WAKUPATTERN[$wWakuPattern]['EndTime'][7];
	$DispPM4 = $aPM4 . "～" . $ePM4;

	$aPM5 = $WAKUPATTERN[$wWakuPattern]['StartTime'][8];
	$ePM5 = $WAKUPATTERN[$wWakuPattern]['EndTime'][8];
	$DispPM5 = $aPM5 . "～" . $ePM5;

	$aPM6 = $WAKUPATTERN[$wWakuPattern]['StartTime'][9];
	$ePM6 = $WAKUPATTERN[$wWakuPattern]['EndTime'][9];
	$DispPM6 = $aPM6 . "～" . $ePM6;


	$cellPM1c = getExcelAddress(($wWakuAM1col + $wWakuAM2col + 4), 8); #列　行
	$cellPM1r = getExcelAddress(($wWakuAM1col + $wWakuAM2col + 3 + $wWakuPM1col), 8); #列　行
	$sheet->mergeCells($cellPM1c . ':' . $cellPM1r);
	$sheet->setCellValue($cellPM1c, $DispPM1);

	$cellPM2c = getExcelAddress(($wWakuAM1col + $wWakuAM2col + 4 + $wWakuPM1col), 8); #列　行
	$cellPM2r = getExcelAddress(($wWakuAM1col + $wWakuAM2col + 3 + $wWakuPM1col + $wWakuPM2col), 8); #列　行
...
$wCellStart++;
$sheet->setCellValue('B' . $wCellStart, "電話 " . $TelNumber . "（日時変更フリーダイヤル）");
...
for ($i = 0; $i < count($wSenyuDate); $i++) {
	...
	for ($j = 0; $j < $wHansu; $j++) {
		...
		$KoteihyouTa_AM = is_countable($KoteihyouTa[$i * $wHansu + $j]['AM']) ? count($KoteihyouTa[$i * $wHansu + $j]['AM']) : 0;
		...
		$KoteihyouTa_AM1 = is_countable($KoteihyouTa[$i * $wHansu + $j]['AM1']) ? count($KoteihyouTa[$i * $wHansu + $j]['AM1']) : 0;
		for ($k = 0; $k < $KoteihyouTa_AM1; $k++) {
			#echo " AM ".$KoteihyouTa[$i*$wHansu+$j]['AM'][$k];
			$KojiDate['RoomID'][] = $KoteihyouTa[$i * $wHansu + $j]['AM1'][$k]; #
			$KojiDate['RoomDate'][] = $wSenyuDate[$i] . " " . $aAM; #日付+AMの開始時刻　196行付近に定義あり
		}
		$KoteihyouTa_AM2 = is_countable($KoteihyouTa[$i * $wHansu + $j]['AM2']) ? count($KoteihyouTa[$i * $wHansu + $j]['AM2']) : 0;
		for ($k = 0; $k < $KoteihyouTa_AM2; $k++) {
			#echo " AM ".$KoteihyouTa[$i*$wHansu+$j]['AM'][$k];
			$KojiDate['RoomID'][] = $KoteihyouTa[$i * $wHansu + $j]['AM2'][$k]; #
			$KojiDate['RoomDate'][] = $wSenyuDate[$i] . " " . $aAM; #日付+AMの開始時刻　196行付近に定義あり
		}
		$KoteihyouTa_PM1 = is_countable($KoteihyouTa[$i * $wHansu + $j]['PM1']) ? count($KoteihyouTa[$i * $wHansu + $j]['PM1']) : 0;
		...
		$KoteihyouTa_PM2 = is_countable($KoteihyouTa[$i * $wHansu + $j]['PM2']) ? count($KoteihyouTa[$i * $wHansu + $j]['PM2']) : 0;

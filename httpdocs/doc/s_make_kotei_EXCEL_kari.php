...
#echo $wWakuAM;
	...
	if($wKojijun==2)
		$myListObject->Order = "cast(RoomID as signed) desc, left(RoomID, 1), left(RoomID, 2), left(RoomID, 3), left(RoomID, 4), left(RoomID, 5), RoomID";
	else
		$myListObject->Order = "cast(RoomID as signed) asc, left(RoomID, 1), left(RoomID, 2), left(RoomID, 3), left(RoomID, 4), left(RoomID, 5), RoomID";
	$myListObject->Limit = "allpage";

	if (!($myListObject->GetList(1)))
		trigger_error("Getting tKojiDateF List Failed.", E_USER_ERROR);

	$KojiDateLoop = $myListObject->Rows;
#		echo "<br>".__LINE__."行目：".$KojiDateLoop;
	for($i = 0; $i < $KojiDateLoop; $i++ ){
		$RoomID[$i] = $myListObject->GetValue($i,0);
		$RoomDate[$i] = $myListObject->GetValue($i,1);
		$RoomTime[$i] = substr($RoomDate[$i],11,5);
		$RoomDay[$i] = substr($RoomDate[$i],0,10);
		$Room[$i] = substr($RoomID[$i],-2);

		$Kai[$i] = substr($RoomID[$i] , 0, strlen($RoomID[$i])-2);

		if($i==0){
			$wKai[] = $Kai[$i];
		}elseif($Kai[$i] != $Kai[$i-1]){
			$wKai[] = $Kai[$i];
		}
		$KaiRoom[$Kai[$i]][] = $RoomID[$i];
	...
	if($wKojijun==1){
		...
		for($i = 0 ; $i < $KojiDateLoop ; $i++ ){
			if($i==0){
				sort($KaiRoom[$Kai[$i]] );
					for($j=0;$j<count($KaiRoom[$Kai[$i]]);$j++)
						$KaiRoom2[] = $KaiRoom[$Kai[$i]][$j];
			}elseif($Kai[$i]!=$Kai[$i-1]){
				sort($KaiRoom[$Kai[$i]]);
					for($j=0;$j<count($KaiRoom[$Kai[$i]]);$j++)
						$KaiRoom2[] = $KaiRoom[$Kai[$i]][$j];
		...
		for($i=1 ; $i <= $wKai[0] ; $i++){
			$y += count($KaiRoom[$i]);
			for($j=0 ; $j < count($KaiRoom[$i]) ; $j++ ){
				$room = substr($KaiRoom[$i][$j],-2);
				$KaiRoom2_kari[$i][$room] = $KaiRoom[$i][$j];
			}
		}
		for($j=0; $j<($y/2);$j++){
			for($i=1 ; $i <= $wKai[0] ; $i++){
#				echo $x;
				for($k=$x;$k<$x+2;$k++){
#					echo sprintf('%02d', $k);
					if($KaiRoom2_kari[$i][sprintf('%02d', ($k+1))]){
						$KaiRoom2[] = $KaiRoom2_kari[$i][sprintf('%02d', ($k+1))];
					}
				}
			}
			$x = $k;
...
//	var_dump($KaiRoom2);
	...
	for($i=0 ;$i<count($KaiRoom2);$i++){
		...
		if (!$myKojiDate->executeSelect("BukkenCD = " . $editBukkenCD." AND RoomID = ".$KaiRoom2[$i], "") ){
			trigger_error("Getting myKojiDate Failed.", E_USER_ERROR);
		}
		$tempdate = substr($myKojiDate->RoomDate,0,10);
		$temptime = substr($myKojiDate->RoomDate,11,5);

		$KaiRoom3_kari[$tempdate][$temptime][] = $myKojiDate->RoomID;

	}
	ksort($KaiRoom3_kari);
	unset($j);
	foreach($KaiRoom3_kari as $first_key => $first_val){
//			echo $first_key;
		for($x=0;$x<$wHansu;$x++){
			for($i=0;$i<count($WAKUPATTERN[$wWakuPattern]['StartTime']);$i++){
				if($i==0){
					$wtemptime = $WAKUPATTERN[$wWakuPattern]['StartTime'][$i];
					$AkiLoop = $wWakuAMcol;
					$wAkiLoop = $wWakuAM;
				}elseif($i==1){
					$wtemptime = $WAKUPATTERN[$wWakuPattern]['StartTime'][$i];
				...
				if(!isset($j[$first_key][$wtemptime])){
					$j[$first_key][$wtemptime]=0;
				}
				if(!isset($m[$first_key][$wtemptime])){
					$m[$first_key][$wtemptime]=0;
				}
				$l=$j[$first_key][$wtemptime];
				for($k=$l ;$k<$AkiLoop+$l ;$k++){
	//				echo "<br>".$m[$first_key][$wtemptime]."<".$wAkiLoop.":::".$first_key.$wtemptime.";;;".$KaiRoom3_kari[$first_key][$wtemptime][$j[$first_key][$wtemptime]];
	//				echo $KaiRoom3_kari[$first_key][$wtemptime][$j[$first_key][$wtemptime]]."<br>".$AkiLoop;
					if($KaiRoom3_kari[$first_key][$wtemptime][$j[$first_key][$wtemptime]]){
						$wKoteihyouEX[]=$KaiRoom3_kari[$first_key][$wtemptime][$j[$first_key][$wtemptime]];
						$j[$first_key][$wtemptime]++;
						$m[$first_key][$wtemptime]++;
					}elseif($m[$first_key][$wtemptime]<$wAkiLoop){
					//	echo "<br>".$k."<".$wAkiLoop;
						$wKoteihyouEX[]="空き";
						$m[$first_key][$wtemptime]++;
					}else{
...
*/
	...
	$myFile->P001 = '詳細工程表_'.date('Ymd_His').'.xlsx';  #ファイル名

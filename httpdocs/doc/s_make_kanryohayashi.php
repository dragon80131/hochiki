...
if ($editBukkenCD > 0) { #物件情報の修正の場合
	...
	if ($myBukkenMatrix->RecCnt == 0 || count($KaiRoom) == 0) { #新規
		...
		$CNT_FILE = "s_make_kanryohayashi.tpl";
...
for ($i = 1; $i <= count($wMaxWakuSuu); $i++) {
	...
	echo ${"wWaku" . $WAKUPATTERN[$wWakuPattern]['AMPM'][$i - 1]};
}
if ($wWakuAM1)
	$wWakuAM = $wWakuAM1;
/*
...
if ($wWakuPattern <= 9) {
	...
	$a4style = "style='display:none'";
	$p4style = "style='display:none'";
	$p5style = "style='display:none'";
	$p6style = "style='display:none'";
} else {
	$a3style = "style='display:block'";
	$a4style = "style='display:block'";
	$p4style = "style='display:block'";
	$p5style = "style='display:block'";
	$p6style = "style='display:block'";
}

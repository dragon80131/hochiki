...
#	include_once _CLS_DIR . "SPUSTanto.cls";
	...
	if ( $_FILES['pic']['error'] == UPLOAD_ERR_OK ){
	    // アップロード先とファイル名を付与
	    $upload_file = "./upload/".date('Ymdhi-'). $_FILES['pic']['name'] ;

	} 
	$kojiwaku1 = SPFWParameter::getValues('kojiwaku1');
	$kojiwaku2 = SPFWParameter::getValues('kojiwaku2');
	$kojiwaku3 = SPFWParameter::getValues('kojiwaku3');
	$kojijun = SPFWParameter::getValues('kojijun');
	$Notes = SPFWParameter::getValues('Notes');
	$pic = SPFWParameter::getValues('pic');

	$data = $_FILES['pic'];
	$tempfile = $_FILES['pic']['tmp_name'];
	$filename = './upload/' .date('Ymdhi-'). $_FILES['pic']['name'];
	$dataName = $_FILES["pic"]["name"];
	$data_array = explode('.',$dataName);
	$extension = strtolower($data_array[count($data_array)-1]);
	
	if ( $extension == "jpeg" or $extension == "jpg" or $extension == "png" or $extension == "gif"){#2
		#写真をフォルダにいれる
		...
		$ErrorStrings[]="写真を選択してください。";
	}else{
		$ErrorStrings[]="写真の形式が正しくありません。";
	}
	if($kojiwaku1=="0"&&$kojiwaku2=="0"&&$kojiwaku3=="0")
		$ErrorStrings[]="工事枠を入力してください。";
	elseif($kojiwaku1!==""&&$kojiwaku2!==""&&$kojiwaku3!=="")
		$kojiwaku = $kojiwaku1."-".$kojiwaku2."-".$kojiwaku3;
	elseif($kojiwaku1!==""&&$kojiwaku2!==""&&!isset($kojiwaku3))
		$kojiwaku = $kojiwaku1."-".$kojiwaku2;
	elseif($kojiwaku1==""&&$kojiwaku2==""&&$kojiwaku3=="")
		$ErrorStrings[]="工事枠を入力してください。";
	else
		$ErrorStrings[]="工事枠の入力が正しくありません。";

	if($kojijun==""){
		$ErrorStrings[]="工事順を選択してください。";
	}elseif($kojijun=="1"){
		$kojijunChecked1='checked';
		$kojijunName="横・下から";
	}elseif($kojijun=="2"){
		$kojijunChecked2='checked';
		$kojijunName="横・上から";
	}elseif($kojijun=="3"){
		$kojijunChecked3='checked';
		$kojijunName="縦・下から";
	}elseif($kojijun=="4"){
		$kojijunChecked4='checked';
		$kojijunName="縦・上から";
	}elseif($kojijun=="5"){
		$kojijunChecked5='checked';
		$kojijunName="特殊";
	}
	...
	if(count($ErrorStrings) > 0){
		...
		unset($ErrorMessage);
		for($i=0; $i < count($ErrorStrings);$i++)
			$ErrorMessage .=$ErrorStrings[$i]. "<br>\n";
		$IfError = TRUE;

		include_once("s_postde.php");
		exit;
	}

...
#機器設定指示書 PATMO・GBM-2M（K） 登録処理
	...
	for($i=1 ; $i < 56 ; $i++){
		...
		if($i<2)#★★
			${"wDenkijoSttei1".$i} = SPFWParameter::getValues("wDenkijoSettei".$i);
		if($i<6)#★
		...
		for($i=0; $i<count($wJutakuServiceTSettei6);$i++ ){
			if($i==0)
				$wJutakuServiceTSettei6Name = $SERVICENAME_VIXUS1Pr1[$wJutakuServiceTSettei6[$i]];
			elseif($i%5==0)
				$wJutakuServiceTSettei6Name .= "<br>,".$SERVICENAME_VIXUS1Pr1[$wJutakuServiceTSettei6[$i]];
			else
				$wJutakuServiceTSettei6Name .= ",".$SERVICENAME_VIXUS1Pr1[$wJutakuServiceTSettei6[$i]];
		}
		for($i=0; $i<count($wJutakuServiceTSettei7);$i++ ){
			if($i==0)
				$wJutakuServiceTSettei7Name = $SERVICENAME_VIXUS1Pr2[$wJutakuServiceTSettei7[$i]];
			elseif($i%5==0)
				$wJutakuServiceTSettei7Name .= "<br>,".$SERVICENAME_VIXUS1Pr2[$wJutakuServiceTSettei7[$i]];
			else
				$wJutakuServiceTSettei7Name .= ",".$SERVICENAME_VIXUS1Pr2[$wJutakuServiceTSettei7[$i]];
		}
		for($i=0; $i<count($wJutakuServiceTSettei8);$i++ ){
			if($i==0)
				$wJutakuServiceTSettei8Name = $SERVICENAME_VIXUS1Pr2[$wJutakuServiceTSettei8[$i]];
			elseif($i%5==0)
				$wJutakuServiceTSettei8Name .= "<br>,".$SERVICENAME_VIXUS1Pr2[$wJutakuServiceTSettei8[$i]];
			else
				$wJutakuServiceTSettei8Name .= ",".$SERVICENAME_VIXUS1Pr2[$wJutakuServiceTSettei8[$i]];
		}
	...
	# ★この機種の初期値レコードを作成してCDの指定を変更する★
	if(!$myKikiSettei->executeSelect("KikiSetteiCD = 4 AND BukkenCD is NULL","")){
...
#echo "6";
	$myKikiSettei = new KikiSettei($myDB);
	#物件CD　機器CD=5:PATMO・GBM-2M（K）　を指定してレコードを検索
	...
	#第二引数はKikisetteiCD　省略すると-1が規定でセット(ecuteUpdateがInsertになる）
	$myKikiSettei = new KikiSettei($myDB);
#echo "$wKikiSetteiCD＝＝＝".$wKikiSetteiCD;
	if($wKikiSetteiCD > 0){
		...
		$myKikiSettei->KikiCD = 5;#PATMO・GBM-2M（K）
	...
	for($i=1 ; $i<56 ;$i++){
		...
		if($i<2)#★★
			$myKikiSettei->{"DenkijoSetteiSettei".$i} = ${"wDenkijoSettei".$i};
		if($i<6)#★
	...
	$CNT_FILE = "s_kikiseteiPATMO_GBM-2M_K_finish.tpl";

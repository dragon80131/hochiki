...
$IfShowHeader1 = $IfShowHeader2 = $IfShowColumn1 = $IfShowColumn2 = $IfDeveloper;
...
$srchDate = SPFWParameter::getValues('srchDate'); // 日程
...
for ($i = 0; $i < $TimeLoop; $i++) { #時間選択し
	...
	if($sAMPMInfo != '')
		$sAMPMInfo .= '<br>';
	if(isset($WAKUPATTERN[$WakuPattern]['AMPM'][$i]) && $wTime[$i])
		$sAMPMInfo .= $WAKUPATTERN[$WakuPattern]['AMPM'][$i].':'.$wTime[$i];
}
...
$sql .= "r.TimeMeaning,";
$sql .= "u.ConfirmFlg,";
$sql .= "DATE_FORMAT(r.TimeFrom, '%c/%e') AS DateFrom";
$myListObject->SelectSQL = $sql;
...
// if($UserKbn != 2){#ユーザ区分が管理者でなければ　自分の幹事企業のみ表示
$sql .= " WHERE u.MukouFlg = FALSE AND u.BukkenCD = " . $editBukkenCD . " AND r.ID IS NOT NULL ";
if($editBuildingCD){
...
if($srchDate){ // 階数による絞り込み
	$sql .= " AND DATE_FORMAT(r.TimeFrom, '%c/%e') = '".date('n/j', strtotime($srchDate))."'";
}
...
for ($i = 0; $i < $ResidentsFormLoop; $i++) {
	...
	$TimeFromDate[$i]	= $myListObject->GetValue($i, 12);

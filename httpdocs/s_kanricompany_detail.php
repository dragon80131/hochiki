...
$isAdminMode = TRUE;
	...
	if ($editGyosyaCD > 0){
		...
		if (!$myKanriCompany->executeSelect("KanriCompanyCD = " . $editGyosyaCD, "") || $myKanriCompany->RecCnt == 0){
			trigger_error("Getting KanriCompany Failed.", E_USER_ERROR);
		}
		$wKanriCompanyCD = $myKanriCompany->KanriCompanyCD;
		$wKanriCompanyName = $myKanriCompany->KanriCompanyName;
		$wKanriCompanyNameKana = $myKanriCompany->KanriCompanyNameKana;
		$wCompanyTEL = $myKanriCompany->CompanyTEL;
		$wNotes = $myKanriCompany->Notes;
	...
	$CNT_FILE = "s_kanricompany_detail.tpl";

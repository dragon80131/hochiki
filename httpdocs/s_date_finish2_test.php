...
if ($work == 1) { #新規、修正
	...
	foreach ($wSagyoDates as $key => $value) {
		$number = $key + 1;
		switch ($number) {
			case 1:
				$myBukken->Date1 = $value;
				break;
			case 2:
				$myBukken->Date2 = $value;
				break;
			case 3:
				$myBukken->Date3 = $value;
				break;
			case 4:
				$myBukken->Date4 = $value;
				break;
			case 5:
				$myBukken->Date5 = $value;
				break;
			case 6:
				$myBukken->Date6 = $value;
				break;
		}
	}

	$myBukken->KikiStatus = STATUS["日程入力済"];
	if ($myBukken->Date1) {
		$UketsukeShimekiriDate_week = date('w', strtotime($myBukken->Date1 . '-2 day'));

		if ($UketsukeShimekiriDate_week == 0) {
			$UketsukeShimekiriDate = date('Y-m-d', strtotime($myBukken->Date1 . '-4 day'));
		} else if ($UketsukeShimekiriDate_week == 6) {
			$UketsukeShimekiriDate = date('Y-m-d', strtotime($myBukken->Date1 . '-3 day'));
		} else {
			$UketsukeShimekiriDate = date('Y-m-d', strtotime($myBukken->Date1 . '-2 day'));
		}
	} else {
		$UketsukeShimekiriDate = "";
	}

	$myBukken->FirstKojiDate = $wSagyoDate;
	$myBukken->LastKojiDate = $wSagyoDate2;
	$myBukken->KojiTime  = $wSagyoTime;
	$myBukken->UketsukeShimekiriDate = $UketsukeShimekiriDate;
	$myBukken->Updater = $UserCD;

	$myBukken->SagyoLastUpdated = date("Y-m-d H:i:s");
	$myBukken->SagyoLastUpdater = $UserCD;
	...
	$title = "【点検システム】" . $BukkenName . "実施日程登録通知";
	$content = $BukkenName . "の実施日程が登録されました\r\nシステムにログインし、内容をご確認ください。\r\n";

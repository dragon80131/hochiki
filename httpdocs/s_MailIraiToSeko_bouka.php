...
try {
	...
	//防災依頼状態にする
	$statement = 'UPDATE tBukkenM SET BousaiStatus = 1 WHERE BousaiTenkenMonth = ' . $TargetMonth;
...
for ($i = 0; $i < $SekoCompanyLoop; $i++) {
	...
	$sql .= " AND ( BousaiStatus = 1 OR BousaiStatus = 0 )"; //施工への依頼中状態と物件作成直後の状態
	...
	if ($GyosyaMail[$i] or $GyosyaMail2[$i]) {
		...
		$title = "【点検システム】" . $TargetMonth . "月点検分　防火実施日登録依頼";
		$content = "システムにログインし、" . $TargetMonth . "月防火点検予定の、実施日程登録をお願いいたします。\r\n";

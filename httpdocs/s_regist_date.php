...
$Last_Updater = $myBukken->SagyoLastUpdater;
$Last_Updated = $myBukken->SagyoLastUpdated;
...
#tReservationFにいれれるように　TimeFrom TimeToをつくる。
// DateTimeオブジェクトを作成
$start = new DateTime($SenyuStartDate);
$end = new DateTime($SenyuEndDate);
// 日付の差を求める
$diff = $start->diff($end);
#WakuPattern の　
// $WAKUPATTERN[3]['Name'] = "3枠( AM 9:00-12:00, PM1 13:00-15:00, PM2 15:00-17:00)";
// $WAKUPATTERN[3]['StartTime'][0] = "09:00";
// $WAKUPATTERN[3]['EndTime'][0] = "12:00";
// $WAKUPATTERN[3]['AMPM'][0] = "AM";
// $WAKUPATTERN[3]['WD60min'][1] = "3";
// $WAKUPATTERN[3]['WE60min'][1] = "2";
...
// $WAKUPATTERN[3]['WD60min'][2] = "3";
// $WAKUPATTERN[3]['WE60min'][2] = "2";
...
if ($Last_Updater) {
	...
	$myUser = new User($myDB);
	if (!$myUser->executeSelect("MukouFlg = FALSE AND UserCD = " . $Last_Updater, "") || $myUser->RecCnt != 1) {
...
$sql .= " AND TenkenKind = " . $NextTenkenKind;
...
for ($i = 0; $i < $KojiNitteiLoop; $i++) {
	$KojiNitteiCD[$i] 	= $myListObject->GetValue($i, 0);
	$RoomNumber[$i]	= $myListObject->GetValue($i, 1);
	...
	if ($i % 2 == 0) {
		if ($NextTenkenKind == 1) {
			$KobetsuTime[$i] = "AM:" . $KikiAMKojiTime;
		} else if ($NextTenkenKind == 2) {
			$KobetsuTime[$i] = "AM:" . $SougouAMKojiTime;
		}
	} else {
		if ($NextTenkenKind == 1) {
			$KobetsuTime[$i] = "PM:" . $KikiPMKojiTime;
		} else if ($NextTenkenKind == 2) {
			$KobetsuTime[$i] = "PM:" . $SougouPMKojiTime;
		}
	}
}
unset($myListObject);
...
# 設定情報
########################################################
/*ここ途中　10/5	$myKoji = new Koji($myDB);

	if (!$myKoji->executeSelect("MukouFlg = FALSE AND ClientCD = " . $ClientCD, "") || $myKoji->RecCnt != 1){
		trigger_error("Getting Koji Failed.", E_USER_ERROR);
	}

	$wKenmeiNo 				= $myKoji->KenmeiNo;
	$wKojiName 			= $myKoji->KojiName;
	$wKojiName_Hurigana	= $myKoji->KojiName_Hurigana;
	$wAddress 				= $myKoji->Address;

	$wKojiMemo 				= $myKoji->KojiMemo;
	*/
/*
	if($myBukken->TantoCD){

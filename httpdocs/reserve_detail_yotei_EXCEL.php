...
if($wArrangeType == '1'){ // 点検
	$FileFormat = './template/yotei_temp_Floor2025.xlsx';
	$spreadsheet = $reader->load($FileFormat);
	...
	$sheet->setCellValue('B4', "～　".$SagyoName."のご案内　～"); #マンション名
	...
	foreach($arrFloorReserveInfo as $floor => $FloorReserveInfo){
		if (!array_key_exists($FloorReserveInfo["wFloorDay"], $arrDateReserveInfo)) {
			$arrDateReserveInfo[$FloorReserveInfo["wFloorDay"]] = array();
		}

		if(!array_key_exists($FloorReserveInfo["wFloorWaku"], $arrDateReserveInfo[$FloorReserveInfo["wFloorDay"]])){
			$arrDateReserveInfo[$FloorReserveInfo["wFloorDay"]][$FloorReserveInfo["wFloorWaku"]] = array();
		}
		array_push($arrDateReserveInfo[$FloorReserveInfo["wFloorDay"]][$FloorReserveInfo["wFloorWaku"]], $FloorReserveInfo["wFloor"]."");
	}

	ksort($arrDateReserveInfo);
	$rowIndex = 14;
	$onePageRows = 0;
	foreach($arrDateReserveInfo as $wFloorDay => $arrAMPMReserveInfo){
		if($onePageRows >= 10 && !$isTwoPage){
			$rowIndex = 52;
			$sheet->setCellValue("A50", "点検日程（前ページからの続き）：");
			$sheet->getStyle("A50")->applyFromArray($styleArray3);

			$sheet->mergeCells("D51:G51");
			$sheet->getStyle("D51:G51")->applyFromArray($styleArray2);
			$sheet->setCellValue("D51", "点検日");

			$sheet->mergeCells("H51:N51");
			$sheet->getStyle("H51:N51")->applyFromArray($styleArray2);
			$sheet->setCellValue("H51", "点検時間");

			$sheet->mergeCells("O51:S51");
			$sheet->getStyle("O51:S51")->applyFromArray($styleArray2);
			$sheet->setCellValue("O51", "点検場所");

			$isTwoPage = true;
		}

		$rowIndexStartDate = $rowIndex;
		$rowIndexEndDate = $rowIndex;
		foreach($arrAMPMReserveInfo as $wFloorWaku => $arrFloorInfo){
			$sheet->mergeCells("H".$rowIndex . ":" . "N".$rowIndex);
			$sheet->getStyle("H".$rowIndex . ":" . "N".$rowIndex)->applyFromArray($styleArray1);
			...
			for ($i = 0; $i < $WakuKazu; $i++) { #時間選択し
				if($WAKUPATTERN[$wWakuPattern]['AMPM'][$i] == $wFloorWaku){
			...
			$sheet->setCellValue("H".$rowIndex, $sTime);

			$sheet->mergeCells("O".$rowIndex . ":" . "S".$rowIndex);
			$sheet->getStyle("O".$rowIndex . ":" . "S".$rowIndex)->applyFromArray($styleArray2);
			$FloorVal = "";
			foreach($arrFloorInfo as $wFloor){
				if($FloorVal != "")
					$FloorVal .= "、";
				$FloorVal .= $wFloor."階";
			}
			$sheet->setCellValue("O".$rowIndex, $FloorVal);
			$rowIndex ++;
			$rowIndexEndDate ++;
			$onePageRows ++;
		}
		if($rowIndexEndDate > $rowIndexStartDate)
			$rowIndexEndDate --;

		$sheet->mergeCells("D".$rowIndexStartDate . ":" . "G".$rowIndexEndDate);
		$sheet->getStyle("D".$rowIndexStartDate . ":" . "G".$rowIndexEndDate)->applyFromArray($styleArray1);
		$sheet->setCellValue("D".$rowIndexStartDate, date("n月j日", strtotime($wFloorDay)));
	...
	if($YoyakuEndDate){
		...
		$sheet->setCellValue('L38', date("n月j日", strtotime($YoyakuEndDate))."(".$YoyakuEndWeek.")");
	}else{
	...
	// 物件管理番号
	$sheet->setCellValue('H39', $editBukkenCD);
	// ワンタイムパスワード
	$sheet->setCellValue('P39', $wPasswd);
	// URL
	$sheet->setCellValue('E40', "URL ：".$TargetURL);

	#QRコード
	if (file_exists($DomainQR_path)) {
	...
	$TantoNameStr = $Tanto1Name;
	if($TantoNameStr != "")
		$TantoNameStr .= "、" . $Tanto2Name;
	else
		$TantoNameStr = $Tanto2Name;

	$sheet->setCellValue('Q43', $TantoNameStr);
...
if($isTwoPage){
	$sheet->getPageSetup()->setPrintArea('A1:T'.($rowIndex+1));
}

<?php
	...
	$TargetMonth = date('m', strtotime('+2 month'));
	// try {
	// 	$options = [
	// 	PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION
	// 	, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC
	// 	];
	// 	$statement = 'UPDATE tBukkenM SET KikiStatus = 1 WHERE KikiTenkenMonth = "'.$TargetMonth.'" OR SougouTenkenMonth = "'.$TargetMonth.'"';
	...
	for($i=0; $i< $SekoCompanyLoop; $i++){
		...
		for ($j = 0; $j < $BukkenLoop; $j++) {
			$BukkenName .= $myListObject2->GetValue($j, 0)."\r\n";
		}
		unset($myListObject2);
		...
		if($GyosyaMail[$i]){
			...
			$to = $GyosyaMail[$i];
			$title = "点検日程登録依頼";
			$content = "点検システムから日程登録をお願い致します。\r\n";
			$content.= $BukkenName."の日程入力をお願いします。\r\n";

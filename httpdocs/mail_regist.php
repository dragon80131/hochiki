...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if ($verificationCode != NULL) {
		// 確認コードが正しいかチェック
		if ($myUser->EmailVerificationCode == $verificationCode && strtotime($myUser->EmailVerificationExpiry) > time()) {
			// 確認コードが正しければ、メールアドレスを確定
			$myUser->EmailVerified = TRUE;
			$myUser->EmailVerificationCode = NULL; // 確認コードをクリア
			$myUser->EmailVerificationExpiry = NULL; // 有効期限もクリア

			if (!$myUser->executeUpdate("", $myUser->UserCD)) {
		...
		$CNT_FILE = "mail_confirm.tpl";
	}
	else if (filter_var($wEMail, FILTER_VALIDATE_EMAIL)) {
		###### 確認メール送信処理 ######

		$myUser->EMail = $wEMail;
		$myUser->EmailVerificationCode = $code;
		$myUser->EmailVerificationExpiry = $codeExpiry;
		...
		sendTwoFactorAuthMail($wEMail, $code, $codeExpiry, $URL);
		$CNT_FILE = "mail_confirm.tpl";
	}
	else {
		// メールアドレスが不正な場合
		$ErrorString = "メールアドレスの形式が正しくありません。";
	}
}

...
function moveAlert(page, key, work, msg) {
		if (window.confirm(" 部屋と予定が登録されていますと削除されます。よろしいでしょうか？ ")) {
			document.mainform.action = page;
			document.mainform.submit(true);
		}
}
...
function moveWithSagyoinCD(page, key) {
	document.mainform.editSagyoinCD.value = key;
	document.mainform.action = page;
	document.mainform.submit(true);
}


function moveWithKeyAndWork3SagyoinCD(page, key, work, msg) {
	if (work == 2){
...
function moveWithBrancheCD(page, key) {
	document.mainform.editBrancheCD.value = key;
	document.mainform.action = page;
	document.mainform.submit(true);
}
...
function moveWithSystemCD(page, key, work) {
	document.mainform2.editSystemCD.value = key;
	document.mainform2.work.value = 1;
	document.mainform2.action = page;
	document.mainform2.submit(true);
}
...
function moveWithKeyAndWork12(page, key, work) {
	if (work == 2){
		if (window.confirm(" 削除してもよろしいでしょうか？ ")) {
			document.mainform.editBrancheCD.value = key;
			document.mainform.work.value = work;
...
function changePage2(page, myPage, allPage) {
	if (myPage == 0)
		myPage = document.mainform.myPage2.value;

	if (myPage < 0 || myPage > allPage)
		alert("ページ指定が不正です。再度確認してみてください。");
	else{
		if (myPage > 0)
			document.mainform.myPage2.value = myPage;
		document.mainform.action = page;
		document.mainform.submit(true);
		return false;
	}

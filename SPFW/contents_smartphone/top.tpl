<html>
<head>
<meta name="viewport" content="width=device-width,initial-scale=1.0,user-scalable=yes" />
<title>リニューアル支援</title>
</head>
__SHeader__
<body bgcolor="__SBackground__" text="__STextColor__" link="__SLinkColor__" alink="__SALinkColor__" vlink="__SVLinkColor__">

<hr size="__HRSize__" color="__HRColor__">
<center>《リニューアル支援》</center>
<hr size="__HRSize__" color="__HRColor__">

<br>
<table align="center"><tr><td align="left" width="100%">

ようこそ、リニューアル支援システムへ<br>
本システムから物件の管理が行えます。<br>
<br>
<font size=2>
システムのご利用はパソコンからのみ可能です。<br>
アプリインストールは以下のリンクをクリック</font>
<br><br>

</td></tr></table>



<table align="center" ><tr><td align="left" width="300">

	__IfLogin__<a href="login_form.php__QUERY__">■ログイン</a><br>__IfLogin__

	__IfReminder__<a href="reminder_form.php__QUERY__">■パスワードを忘れた方</a><br>__IfReminder__

	__IfLogout__<a href="logout.php__QUERY__">■ログアウト</a><br>__IfLogout__

</td></tr></table>


<br><br>


<hr size="__HRSize__" color="__HRColor__">
__SFooter__
__SCopyright__
</body>
</html>

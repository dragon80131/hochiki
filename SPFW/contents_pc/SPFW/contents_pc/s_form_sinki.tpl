...
<head>
	...
	<script>
		...
		// 基本システムの選択と連動して、「室内親機品番」「玄関子機品番」を変更させる
		$(function () {
			//selectタグ（親） が変更された場合
			$("#systems").on('change', function () {
				var systemsVal = $(this).val();
				//ここまではjqueryのトリガー
				$.ajax({ // Ajax通信処理
						//systemsVal値 を getBasicSystems.php へ渡す
						url: "./getBasicSystems.php", // リクエスト送信先URLの設定
						type: "POST", //ポストで渡す
						dataType: "json", //データ形式をJSONとして定義
						data: {
							"Category": systemsVal
						},
					})
					...
					.done(function (data) {
						// 通信成功時の処理を記述
						$("#OyakatabanList").html(data[0]);
						$("#KokikatabanList").html(data[1]);
		...
		$(function () {
			$("#OyakatabanList").select2({
				language: "ja"
			})
			$("#KokikatabanList").select2({
				language: "ja"
			})
		});
	</script>
						...
						<tr id="wOyaKataban">
							...
							<td>
								...
								<select name="wOyaKataban">
									<option value="">-</option>
									__OyaKatabanLoop__
									<option value="__OyaDeviceKataban__" __OyaDeviceKatabanSelected__></option>__OyaDeviceKataban__　__OyaDeviceName__
									__OyaKatabanLoop__
								</select>

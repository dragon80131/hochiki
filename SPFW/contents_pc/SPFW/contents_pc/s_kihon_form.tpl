...
<head>
	...
	<script>
		...
		//型番自由入力
		$(function () {
			var a = $('input[name = "ziyu"]').prop('checked');
			if (a == true) {
				$("#wOyaKataban2").show();
				$("#wOyaKataban").hide();
				$("#wKokiKataban2").show();
			...
			$('input[name = "ziyu"]').change(function () {
				...
				if (a == true) {
					$("#wOyaKataban2").show();
					$("input[name = 'wOyaKataban2']").prop("disabled", false);
					$("#wKokiKataban2").show();
					$("input[name = 'wKokiKataban2']").prop("disabled", false);

					$("select[name = 'wOyaKataban']").prop("disabled", true);
					$("#wOyaKataban").hide();
					$("select[name = 'wKokiKataban']").prop("disabled", true);
					$("#wKokiKataban").hide();

					$("#ziyuflg").val("1");
				} else {
					$("#wOyaKataban").show();
					$("select[name = 'wOyaKataban']").prop("disabled", false);
					$("#wKokiKataban").show();
					$("select[name = 'wKokiKataban']").prop("disabled", false);

					$("input[name = 'wOyaKataban2']").prop("disabled", true);
					$("#wOyaKataban2").hide();
					$("input[name = 'wKokiKataban2']").prop("disabled", true);
					$("#wKokiKataban2").hide();
...
-->
	...
	-->
					...
					<tr>
						<th class="yb">住戸数(半角数字)<font color="red">　※</font>
						</th>
					...
					<tr>
						...
						<td><input type="text" name="wKojiShozokuName" value="__wSekoShutai__" style="width:400px;"></td>
					</tr>
					<tr>
						<th class="yb">会社TEL<font color="red">　※</font>
						</th>
						<td><input type="tel" name="wKojiShozokuTEL" value="__wKojiShozokuTEL__" style="width:150px;"></td>
					</tr>
					<tr>
						<th class="yb">担当名<font color="red">　※</font>
						</th>
						<td><input type="text" name="wKojiTantoName" value="__wKojiTantoName__" style="width:300px;"></td>
					</tr>
					<tr>
...
<tr><th class="yb">宅配連動<font color="red">　※</font></th>
	...
	-->
				...
				◆型番情報
				<input type="checkbox" name="ziyu" id="" value="1" class="ziyu" __checked_flg__>自由入力
				<table class="table table-bordered table-sm" id="" style="width: 800px">
					...
					<tr id="wOyaKataban2">
						...
						<td><input type="text" name="wOyaKataban2" value="__wOyaKataban__"></td>
					</tr>
					<tr id="wKokiKataban2">
						<th class="yb">玄関子機品番(自由入力)</th>
						<td><input type="text" name="wKokiKataban2" value="__wKokiKataban__"></td>
					</tr>
...
<tr><th class="yb">★★★子機パネル</th>
	<td><input type="radio" name="wKokiPanel" value="1" __KokiPanelChecked1__>なし　
		<input type="radio" name="wKokiPanel" value="0" __KokiPanelChecked0__>有り　
				...
				<input type="button" value="登録する" onClick="return required_check(2)" class="btn btn-primary">
	...
	<script>
		...
		function form_check(val) {
			...
			*/
			if (!chk_input("wKosu")) html.push("住戸数が入力されていません。もしくは半角数字以外で入力されています。");
			if (!chk_input("wKaidaka")) html.push("階高が入力されていません。もしくは半角数字以外で入力されています。");
			if (!chk_input("wKojiShozokuName")) html.push("所属名が入力されていません。");
			if (!chk_input("wKojiShozokuTEL")) html.push("所属TELが入力されていません。もしくは半角数字以外で入力されています。");
			...
			*/
			...
			var wKyoyoStartDate = document.getElementsByName("wKyoyoStartDate").item(0).value;
			var wKyoyoEndDate = document.getElementsByName("wKyoyoEndDate").item(0).value;
			var wSenyuStartDate = document.getElementsByName("wSenyuStartDate").item(0).value;
			var wSenyuEndDate = document.getElementsByName("wSenyuEndDate").item(0).value;

			if (wKyoyoStartDate > wKyoyoEndDate) {
				html.push("共用部の開始日が終了日より後の日付になっています");
			}

			if (wSenyuStartDate > wSenyuEndDate) {
				html.push("専有部の開始日が終了日より後の日付になっています");
			}
